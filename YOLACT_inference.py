...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    target_fps   = 30 #round(vid.get(cv2.CAP_PROP_FPS))
    frame_width  = 640 #width = color.get_width() round(vid.get(cv2.CAP_PROP_FRAME_WIDTH))
    frame_height = 480 #round(vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    def get_next_frame(pipeline):

        #Changed for IntelRealse 
        frames = pipeline.wait_for_frames()
        aligned_frames = align.process(frames)
        color_frame = aligned_frames.get_color_frame()
        frames = [np.asanyarray(color_frame.get_data())]
        # frames = []
    ...
    aligned_frames = align.process(frames)
    color_frame = aligned_frames.get_color_frame()
    first_batch = eval_network(transform_frame([np.asanyarray(color_frame.get_data())]))
    print('Done.')
    ...
    try:
        while True and running: #Add check on intelrealsense camera
        #while vid.isOpened() and running:
            ...
            if not vid_done:
                next_frames = pool.apply_async(get_next_frame, args=(pipeline,))
            else:
                next_frames = None
